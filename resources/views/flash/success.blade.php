@if (session('flash.type') === 'success') 
    <div class="flex items-center justify-center px-10 sm:p-0">
        <div
        x-data="{ show: true }"
        x-init="setTimeout(() => { show = 1000 }, 2000)"
        x-show="show"
        ...
        class="notification bg-green-800 p-3 rounded-full px-5 text-xs sm:text-sm {{ request()->routeIs('double.*')?'lg:ms-52 2xl:ms-52' : 'lg:ms-52' }} text-neutral-300 inline-block mx-auto fixed bottom-0 mb-10 text-center shadow-lg z-[100]">
