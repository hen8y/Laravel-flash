@if (session('flash.type') === 'success')
    <div class="flex items-center w-full justify-center px-10 sm:p-0 z-[1000] ">
        ...
        >
            ...
            <div class=" p-3 rounded-full  w-full px-5 text-xs sm:text-sm text-white inline-block mx-auto mb-10 text-center shadow-lg z-[1000] {{ config('flash.enable_dark_mode') === true ? 'block dark:hidden' : '' }} " style="background-color: {{ config('flash.theme.success') }}">
            ...
            <div class="p-3 rounded-full px-5 text-xs w-full sm:text-sm text-white mx-auto mb-10 text-center shadow-lg z-[1000] {{ config('flash.enable_dark_mode') === true ? 'hidden dark:inline-block' : 'hidden' }} " style="background-color: {{ config('flash.dark-theme.success') }}">
                <div class="flex gap-4 items-center min-w-52  w-full max-w-[320px] sm:max-w-screenjustify-between">
                    <p>{{ session('flash.message') }}</p>
